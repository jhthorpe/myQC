...
!---------------------------------------------------------------------
  REAL(KIND=8) FUNCTION BoysG(T)
    ...
    IF (T .GE. 12.0D0 .AND. T .LT. 15.0D0) THEN
      BoysG = 0.4999489092D0-0.2473631686D0*T**(-1.0D0)+0.321180909D0*T**(-2.0D0) &
      -0.3811559346D0*T**(-3.0D0)
    ELSE IF (T .GE. 15.0D0 .AND. T .LT. 18.0D0) THEN
      BoysG = 0.4998436875D0-0.24249438D0*T**(-1.0D0)+0.24642845D0*T**(-2.0D0)
    ELSE IF (T .GE. 18.0D0 .AND. T .LT. 24.0D0) THEN
      BoysG = 0.499093162D0-0.2152832D0*T**(-1.0D0)
